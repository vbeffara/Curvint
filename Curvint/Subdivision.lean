...
theorem mem_iff (σ : Subdivision a b) : t ∈ σ.toList ↔ ∃ i, σ i = t := by
  rw [mem_iff_get]
...
theorem piece_subset {i : Fin (σ.size + 1)} (hab : a ≤ b) : σ.piece i ⊆ Icc a b :=
...
theorem le_mesh {i : Fin (σ.size + 1)} : σ.y i - σ.x i ≤ σ.mesh := by
  apply Finset.le_max' _ _ (Finset.mem_image_of_mem _ (Finset.mem_univ i))

@[simp] theorem union0 {α : Type*} {s : Fin 1 → Set α} : ⋃ i : Fin 1, s i = s 0 := ciSup_unique

theorem cover1 (n : ℕ) (f : Fin (n + 2) → ℝ) (hf : Monotone f) :
...
theorem cover2 (n : ℕ) (f : Fin (n + 2) → ℝ) : Icc (f 0) (f (Fin.last (n + 1))) ⊆
...
theorem cover_aux (n : ℕ) (f : Fin (n + 2) → ℝ) (hf : Monotone f) :
    ⋃ i : Fin (n + 1), Icc (f i.castSucc) (f i.succ) = Icc (f 0) (f (Fin.last (n + 1))) :=
  subset_antisymm (cover1 n f hf) (cover2 n f)

theorem cover (σ : Subdivision a b) (hab : a < b) : ⋃ i : _, σ.piece i = Icc a b := by
  simp only [piece, x, y]
  convert cover_aux _ _ (mono hab).monotone
  simp

theorem cover' (hab : a < b) (t : Icc a b) : ∃ i, ↑t ∈ σ.piece i := by
  rcases t with ⟨t, ht⟩
...
theorem toList_le_of_le (h : σ ≤ τ) : σ.toList ⊆ τ.toList := by
...
theorem sub_le_sub_of_Icc (hab : a ≤ b) (h : Icc a b ⊆ Icc c d) : b - a ≤ d - c := by
  linarith [(Icc_subset_Icc_iff hab).1 h]

theorem mesh_antitone (hab : a < b) : Antitone (mesh : Subdivision a b → ℝ) := by
  intro σ τ hστ
...
theorem aux_mem_Ioo (hab : a < b) (h : i < n) : aux a b n (i + 1) ∈ Ioo a b := by
  constructor
...
@[simp] theorem size (hab : a < b) : (regular hab n).size = n := by
...
@[simp] theorem length_eq (hab : a < b) {i : Fin _} :
    length (regular hab n) i = (b - a) / (n + 1) := by
  have (i x : ℝ) : (i + 1) * x - i * x = x := by ring
  simp [length, aux, this, x, y]

@[simp] theorem lengths_eq (hab : a < b) : lengths (regular hab n) = { (b - a) / (n + 1) } := by
...
theorem isAdapted_of_mesh_lt (hab : a < b) (h1 : ∀ i, IsOpen (S i)) (h2 : Icc a b ⊆ ⋃ i, S i) :
...
theorem sum_eq_zero (h : ∀ i, F i (σ.x i) (σ.y i) = 0) : σ.sum F = 0 :=
  Finset.sum_eq_zero (λ i _ => h i)

theorem sum_congr (h : ∀ i, F i (σ.x i) (σ.y i) = G i (σ.x i) (σ.y i)) :
    σ.sum F = σ.sum G :=
